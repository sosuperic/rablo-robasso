...
# dataset = SketchDataset(hp.data_location, transform=transform)
...
class DecoderRNN(nn.Module):
    ...
    def forward(self, inputs, z, hidden_cell=None):
        ...
        params = torch.split(y,6,1)     # splits into tuple along 1st dim; tuple of num_mixture [(max_seq_len + 1) * batch_size, 6]'s, 1 [(max_seq_len + 1) * batch_size, 3]
...
class SketchModel():
    ...
    def preprocess_batch_from_data_loader(self, batch, lengths):
        ...
        lengths: list of ints 
        """
        batch.transpose_(0, 1)  # Dataloader returns batch in 1st dimension, rest of code expects it to be 2nd
        if use_cuda:
            batch = Variable(batch.cuda().float())
        else:
    ...
    def train(self, epoch):
        ...
        for i, (batch, lengths) in enumerate(data_loader):
            batch, lengths = self.preprocess_batch_from_data_loader(batch, lengths)     # batch = [max_seq_len, batch_size, 5]
            ...
            z_stack = torch.stack([z] * (Nmax+1), dim=0)    # expand z to be ready to concatenate with inputs; [max_seq_len + 1, batch_size, Nz]
            inputs = torch.cat([batch_init, z_stack],2)     # each input is stroke + z; [max_seq_len + 1, batch_size, Nz + 5]
    ...
    def sample_next_state(self):
        ...
        return Variable(next_state).view(1,1,-1),\
                   x, y, q_idx==1, q_idx==2
